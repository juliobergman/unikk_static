...
export default {
  ...
  methods: {
    init() {
      ...
      const ControlscenterPosition = this.controls.target.clone()
      ControlscenterPosition.y = 0
      const ControlsgroundPosition = this.camera.position.clone()
      ControlsgroundPosition.y = 0
      // const e = ControlscenterPosition.distanceTo(ControlsgroundPosition);

      // const Controlsorigin = new THREE.Vector2(this.controls.target.y, 0);
      // const Controlsremote = new THREE.Vector2(-3, e); // replace 0 with raycasted ground altitude
      // const ControlsangleRadians = Math.atan2(
